...
def extract_to_html(img_path, output_dir):
    print("extract_to_html")

    # table_sys = TableSystem.__call__()
    # img_num = len(image_file_list)

    f_html = open(os.path.join(output_dir, "show.html"), mode="w", encoding="utf-8")
    f_html.write("<html>\n<body>\n")
    ...
    starttime = time.time()
    pred_res, _ = TableSystem.__call__(img=img)
    pred_html = pred_res["html"]
    logger.info(pred_html)
    elapse = time.time() - starttime
    logger.info("Predict time : {:.3f}s".format(elapse))
    if len(pred_res["cell_bbox"]) > 0 and len(pred_res["cell_bbox"][0]) == 4:
        img = predict_strture.draw_rectangle(img_path, pred_res["cell_bbox"])
    else:
        img = utility.draw_boxes(img, pred_res["cell_bbox"])
    img_save_path = os.path.join(output_dir, os.path.basename(img_path))
    cv2.imwrite(img_save_path, img)
    f_html.write("<tr>\n")
    f_html.write(f"<td> {os.path.basename(img_path)} <br/>\n")
    f_html.write(f'<td><img src="{img_path}" width=640></td>\n')
    f_html.write(
        '<td><table  border="1">'
    ...
    f_html.write(f'<td><img src="{os.path.basename(img_path)}" width=640></td>\n')
    f_html.write("</tr>\n")
    f_html.write("</table>\n")
    f_html.close()
    return os.path.join(output_dir, "show.html")


def main(args):
    print("lewat main")
    image_file_list = get_image_file_list(args.image_dir)
    # image_file_list = get_image_file_list("F:/Programming/Python/Flask/TIE/PaddleOCR/ppstructure/table/table3.jpg")
